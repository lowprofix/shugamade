  ...
  try {
    ...
    if (!bookingData.title || !bookingData.start || !bookingData.end) {
      return NextResponse.json(
        { 
          success: false, 
          error: 'Données manquantes. Les champs title, start et end sont requis.' 
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      response = await fetch('https://n8n.bienquoi.com/webhook/create-envent', {
        method: 'POST',
        headers: {
      ...
      try {
        ...
        response = await fetch('https://n8n.bienquoi.com/webhook-test/create-envent', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      try {
        ...
        return NextResponse.json(
          { 
            ...
            details: 'Impossible de contacter le serveur n8n après plusieurs tentatives',
    ...
    if (responseData && responseData.success) {
      ...
      const bookingResult = await addBooking(bookingData.start, bookingData.end);
      
      if (!bookingResult.success) {
        console.warn('La réservation a été créée dans n8n mais n\'a pas pu être enregistrée localement:', bookingResult.message);
      }
