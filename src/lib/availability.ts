...
function getCurrentDateInTimezone(): Date {
  ...
  console.log('Date dans le fuseau horaire spécifié:', format(nowInTimezone, 'yyyy-MM-dd HH:mm:ss', { timeZone: TIMEZONE }));
 ...
 */
function generateOptimizedSlots(date: Date, durationMinutes: number, bookings: Booking[]): TimeSlot[] {
  // Vérifier si le jour est un jour d'ouverture
  const dayOfWeek = date.getDay(); // 0 = Dimanche, 1 = Lundi, etc.
  if (!OPEN_DAYS.includes(dayOfWeek)) {
    return []; // Retourner un tableau vide si le jour n'est pas un jour d'ouverture
  ...
  for (let i = 0; i < timeSlots.length - 1; i++) {
    ...
    const availableMinutes = (nextStart.getTime() - currentEnd.getTime() - safetyMarginMs) / 60000;
    ...
    if (availableMinutes >= durationMinutes) {
      ...
      while (slotStart.getTime() + durationMinutes * 60000 <= nextStart.getTime() - safetyMarginMs) {
 ...
 */
function roundToNextMinutesMultiple(date: Date, minutesMultiple: number = 5): Date {
...
function isSlotAvailable(slot: TimeSlot, bookings: Booking[]): boolean {
  ...
  const isOverlapping = bookings.some(booking => {
    ...
    const overlaps = (slotStart < bookingEnd.getTime() + safetyMarginMs) && (slotEnd > bookingStart.getTime() - safetyMarginMs);
    
    if (overlaps) {
      console.log(`Chevauchement détecté: ${format(slot.start, 'HH:mm', { timeZone: TIMEZONE })} - ${format(slot.end, 'HH:mm', { timeZone: TIMEZONE })} chevauche ${format(new Date(bookingStart.getTime()), 'HH:mm', { timeZone: TIMEZONE })} - ${format(new Date(bookingEnd.getTime()), 'HH:mm', { timeZone: TIMEZONE })}`);
    }
 ...
 */
function formatAvailableSlots(slots: TimeSlot[], durationMinutes: number): AvailableSlot[] {
...
async function fetchBookings(): Promise<Booking[]> {
  try {
    const response = await fetch('https://n8n.bienquoi.com/webhook/7357a68b-f6ab-4480-892d-79793c327667', {
    ...
    console.log(`Récupération de ${bookings.length} réservations depuis l'API n8n`);
    ...
    console.log(`${validBookings.length} réservations valides sur ${bookings.length} récupérées`);
    ...
    validBookings.forEach((booking, index) => {
      try {
        ...
        if (start && end) {
          console.log(`Réservation ${index + 1}: ${format(start, 'yyyy-MM-dd HH:mm', { timeZone: TIMEZONE })} - ${format(end, 'HH:mm', { timeZone: TIMEZONE })}`);
        }
      } catch (error) {
        console.warn(`Erreur lors de l'affichage de la réservation ${index + 1}:`, error);
      }
    });
 ...
 */
export async function checkSlotAvailability(start: Date, end: Date): Promise<{ available: boolean; reason?: string }> {
  try {
    console.log('Vérification de la disponibilité du créneau:');
    console.log('- Date de début du créneau:', format(start, 'yyyy-MM-dd HH:mm:ss', { timeZone: TIMEZONE }));
    ...
    if (startHour < OPEN_HOUR || (endHour > CLOSE_HOUR || (endHour === CLOSE_HOUR && endMinutes > 0))) {
      return { available: false, reason: 'Le créneau est en dehors des horaires d\'ouverture' };
    }
    ...
    const isOverlapping = bookings.some(booking => {
      ...
      const overlaps = (start.getTime() < bookingEnd.getTime() + safetyMarginMs) && 
                       (end.getTime() > bookingStart.getTime() - safetyMarginMs);
      
      if (overlaps) {
        console.log(`Chevauchement détecté dans checkSlotAvailability:`);
        console.log(`- Nouveau créneau: ${format(start, 'yyyy-MM-dd HH:mm:ss', { timeZone: TIMEZONE })} - ${format(end, 'yyyy-MM-dd HH:mm:ss', { timeZone: TIMEZONE })}`);
        console.log(`- Réservation existante: ${format(new Date(bookingStart.getTime()), 'yyyy-MM-dd HH:mm:ss', { timeZone: TIMEZONE })} - ${format(new Date(bookingEnd.getTime()), 'yyyy-MM-dd HH:mm:ss', { timeZone: TIMEZONE })}`);
        console.log(`- Condition 1: ${start.getTime() < bookingEnd.getTime() + safetyMarginMs}`);
        console.log(`- Condition 2: ${end.getTime() > bookingStart.getTime() - safetyMarginMs}`);
      }
    ...
    if (isOverlapping) {
      return { available: false, reason: 'Le créneau chevauche une réservation existante' };
    }
    ...
    return { available: false, reason: 'Erreur lors de la vérification du créneau' };
  }
}
 ...
 */
export async function getAvailableSlots(durationMinutes: number, daysToCheck: number = 7, startDateStr: string = ''): Promise<AvailableSlot[]> {
  ...
  console.log('Date de début pour la recherche de créneaux:', format(startDate, 'yyyy-MM-dd HH:mm:ss', { timeZone: TIMEZONE }));
  ...
  for (let i = 0; i < daysToCheck; i++) {
    ...
    if (i === 0) {
      ...
      dailySlots.slice(0, 5).forEach(slot => {
        console.log(`- ${format(slot.start, 'HH:mm', { timeZone: TIMEZONE })} - ${format(slot.end, 'HH:mm', { timeZone: TIMEZONE })}`);
      });
    }
    ...
    const formattedSlots = formatAvailableSlots(availableDailySlots, durationMinutes);
 ...
 */
export async function addBooking(startStr: string, endStr: string): Promise<{ success: boolean; message: string }> {
  try {
    ...
    console.log(`Dates normalisées: ${format(start, 'yyyy-MM-dd HH:mm:ss', { timeZone: TIMEZONE })} - ${format(end, 'yyyy-MM-dd HH:mm:ss', { timeZone: TIMEZONE })}`);
    ...
    if (hasOverlap) {
      ...
      return { 
        success: false, 
        message: "Le créneau chevauche une réservation existante (détecté lors de la double vérification)" 
      };
    }
    ...
    return { 
      success: false, 
      message: `Erreur lors de l'ajout de la réservation: ${error instanceof Error ? error.message : "Erreur inconnue"}` 
    };
  }
}
