  ...
  try {
    ...
    if (isNaN(duration) || duration <= 0 || isNaN(days) || days <= 0) {
      return NextResponse.json(
        { error: 'Paramètres invalides. La durée et le nombre de jours doivent être des nombres positifs.' },
        { status: 400 }
      );
    }
    
    // Récupérer les créneaux disponibles
    const availableSlots = await getAvailableSlots(duration, days, startDateStr);
  ...
  try {
    ...
    if (!startTime || !endTime) {
      return NextResponse.json(
        { 
          success: false, 
          error: 'Paramètres manquants. Les paramètres startTime et endTime sont requis.' 
        },
        { status: 400 }
      );
    }
