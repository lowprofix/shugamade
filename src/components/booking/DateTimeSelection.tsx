...
export default function DateTimeSelection({ 
  ...
  const fetchAvailableSlots = async (durationMinutes: number, startDate?: string, additionalDays: number = 30) => {
    ...
    try {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 500) {
            throw new Error(`Erreur serveur: Le serveur a rencontré un problème interne.`);
          } else if (response.status === 404) {
            throw new Error(`Erreur: L'API de réservation n'est pas disponible.`);
          } else {
        ...
        console.log(`Récupéré ${slots.length} créneaux disponibles:`, slots.slice(0, 3));
      ...
      if (err.name === 'AbortError') {
        setError("Le serveur met trop de temps à répondre. Veuillez réessayer ultérieurement.");
      } else if (err.message?.includes('fetch failed') || err.cause?.code === 'ENOTFOUND') {
        setError("Impossible de se connecter au serveur de réservation. Le serveur semble être indisponible. Veuillez réessayer plus tard ou contacter le support.");
      } else if (err.message?.includes('Erreur serveur')) {
        setError("Le serveur de réservation a rencontré un problème. Veuillez réessayer plus tard ou contacter le support.");
      } else {
        setError("Une erreur est survenue lors de la récupération des créneaux disponibles. Veuillez réessayer.");
      }
    } finally {
      setIsLoading(false);
  ...
  const loadMoreSlots = (month: Date) => {
    ...
    if (loadedMonths.some(m => m.getMonth() === month.getMonth() && m.getFullYear() === month.getFullYear())) {
      return;
    }
  ...
  const handleMultipleSlotSelection = (slot: AvailableSlot) => {
    ...
    const serviceTypeStr = service.name.includes("Tempes") ? "Tempes" : "Tête entière";
    ...
    if (isSelected) {
      ...
      if (selectedSlots.length < sessionsNeeded) {
        ...
        setValidationError(`Vous ne pouvez sélectionner que ${sessionsNeeded} séances pour ce forfait.`);
  ...
  const handleConfirmMultipleSlots = () => {
    ...
    if (selectedSlots.length !== sessionCount) {
      setValidationError(`Veuillez sélectionner exactement ${sessionCount} séances.`);
        ...
        <div className="flex items-start">
          ...
          <div>
            ...
            <div className="flex items-center mt-1 text-sm text-gray-600 dark:text-gray-300">
              ...
              <span className="font-medium text-[#ffb2dd]">{service.price}</span>
            </div>
            {isMultipleBooking && (
              <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                Veuillez sélectionner {service.name.includes("4 séances") ? "4" : "6"} créneaux pour ce forfait.
              </p>
            )}
          </div>
        </div>
        ...
        <CardContent className="p-0">
          {isLoading && !loadingMoreSlots ? (
            <div className="flex flex-col items-center justify-center p-12">
              ...
              <p className="text-gray-600 dark:text-gray-300">Chargement des disponibilités...</p>
            </div>
          ) : error ? (
              ...
              <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-3">
                  ...
                  <div className="flex-1 text-center sm:text-left">
                    <h3 className="text-base sm:text-lg font-medium text-red-800 dark:text-red-300 mb-1 sm:mb-2">Problème de connexion</h3>
                    <p className="text-sm sm:text-base text-red-600 dark:text-red-400 mb-2 sm:mb-3">{error}</p>
                    ...
                    <Button 
                      onClick={() => fetchAvailableSlots(service.durationMinutes || 30)}
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* Calendrier */}
                <div>
                  <h4 className="mb-3 text-sm font-medium text-gray-700 dark:text-gray-300">Sélectionnez une date</h4>
                  <div className="relative">
                    ...
                    <div className="block lg:hidden">
                      <Calendar
                        ...
                        disabled={[
                          { before: new Date() },
                          (date) => !availableDatesObj.some(d => isSameDay(d, date))
                        ]}
                        locale={fr}
                        numberOfMonths={1}
                        className="rounded-lg border border-gray-200 dark:border-gray-800 p-3"
                        classNames={{
                          day_selected: "bg-[#bfe0fb] text-white hover:bg-[#9deaff] hover:text-white focus:bg-[#9deaff] focus:text-white",
                          ...
                          day_range_middle: "bg-[#bfe0fb]/20 text-gray-900 dark:text-gray-100",
                    ...
                    {/* Calendrier pour desktop (2 mois) */}
                ...
                <div>
                  <h4 className="mb-3 text-sm font-medium text-gray-700 dark:text-gray-300">Sélectionnez un horaire</h4>
                  {selectedDate ? (
                    slotsForSelectedDate.length === 0 ? (
                      ...
                      <>
                        <div className="mb-3 text-sm text-gray-600 dark:text-gray-300">
                          <p className="font-medium">{formatDateString(selectedDate)}</p>
                          <p className="text-xs mt-1">Chaque créneau a une durée de {service.durationMinutes} minutes</p>
                        </div>
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                          {slotsForSelectedDate.map((slot, index) => {
                            ...
                            const selectedIndex = isMultipleBooking 
                              ? selectedSlots.findIndex(s => s.date === slot.date && s.start === slot.start)
                              : -1;
